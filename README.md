...
`toc_entries` (`source_id`, `entry_id`, `title`, `level`, `position_ref`, `sort_order`) captures a freeform source's table of contents at import time — currently populated by the EPUB importer from `nav.xhtml`/`toc.ncx`, one row per heading pointing at the `entries` row it resolves to, preserving nesting via `level`. Rendered as a jump-to dropdown in an imported text's dedicated pane.
