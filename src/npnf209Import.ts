...
export const NPNF209_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 9: Hilary of Poitiers, John of Damascus';

const BUNDLE_URL = '/library/patristic/npnf209.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — Hilary's De
// Trinitate and Homilies on the Psalms, each an editor's introduction beside
// the work itself, and John of Damascus's four Books of the Exposition.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-9 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-9 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF209_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF209(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF209_TITLE, type: 'extra-biblical', language: 'en',
