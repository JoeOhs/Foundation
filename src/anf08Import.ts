...
export const ANF08_TITLE =
  'Ante-Nicene Fathers, Vol. 8: Fathers of the Third and Fourth Centuries (Continued)';

const BUNDLE_URL = '/library/patristic/anf08.json';
...
async function loadBundle(): Promise<BundledANF08File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF08 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF08File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF08 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF08File): ParsedSource {
...
export async function installANF08(onProgress: (msg: string) => void): Promise<number> {
