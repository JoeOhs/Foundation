...
async function loadBundle(): Promise<BundledANF04File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF04 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF04File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF04 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF04File): ParsedSource {
...
export async function installANF04(onProgress: (msg: string) => void): Promise<number> {
