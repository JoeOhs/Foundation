...
async function loadBundle(): Promise<BundledANF01File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF01 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF01File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF01 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF01File): ParsedSource {
...
export async function installANF01(onProgress: (msg: string) => void): Promise<number> {
