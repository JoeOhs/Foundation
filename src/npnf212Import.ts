...
export const NPNF212_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 12: Leo the Great, Gregory the Great (Part I)';

const BUNDLE_URL = '/library/patristic/npnf212.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — the four
// Parts of Gregory's Book of Pastoral Rule and Books I–VIII of his Register
// of the Epistles.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-12 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-12 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF212_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF212(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF212_TITLE, type: 'extra-biblical', language: 'en',
