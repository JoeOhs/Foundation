...
export const ANF02_TITLE =
  'Ante-Nicene Fathers, Vol. 2: Fathers of the Second Century';

const BUNDLE_URL = '/library/patristic/anf02.json';
...
async function loadBundle(): Promise<BundledANF02File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF02 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF02File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF02 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF02File): ParsedSource {
...
export async function installANF02(onProgress: (msg: string) => void): Promise<number> {
