...
export const NPNF207_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 7: Cyril of Jerusalem, Gregory Nazianzen';

const BUNDLE_URL = '/library/patristic/npnf207.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — here the
// nine correspondent headings under Gregory Nazianzen's Miscellaneous
// Letters, which hold seventy-two letters between them.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-7 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-7 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF207_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF207(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF207_TITLE, type: 'extra-biblical', language: 'en',
