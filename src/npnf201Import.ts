...
const BUNDLE_URL = '/library/patristic/npnf201.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — Vol. 1's Life
// of Constantine and its four Books. Series I never needed it: no volume
// there ran deeper than Work → Chapter.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-1 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-1 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF201_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF201(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF201_TITLE, type: 'extra-biblical', language: 'en',
