...
// has none — it is the only one of the batch whose markup stops at div3 —
// but the field is part of the Series II bundle shape, so it is handled
// here as well.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-2 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-2 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF202_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF202(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF202_TITLE, type: 'extra-biblical', language: 'en',
