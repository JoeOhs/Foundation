...
export const NPNF203_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 3: Theodoret, Jerome and Gennadius, Rufinus and Jerome: Historical Writings';

const BUNDLE_URL = '/library/patristic/npnf203.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — Theodoret's
// Church History and the two Apologies, each a container of Books. Series I
// never needed it: no volume there ran deeper than Work → Chapter.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-3 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-3 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF203_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF203(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF203_TITLE, type: 'extra-biblical', language: 'en',
