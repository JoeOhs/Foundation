...
export const NPNF204_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 4: Athanasius: Select Works and Letters';

const BUNDLE_URL = '/library/patristic/npnf204.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — this volume's
// Festal and Personal Letters, and the four Discourses against the Arians.
// Series I never needed it: no volume there ran deeper than Work → Chapter.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-4 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-4 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF204_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF204(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF204_TITLE, type: 'extra-biblical', language: 'en',
