...
export const NPNF211_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 11: Sulpitius Severus, Vincent of Lérins, John Cassian';

const BUNDLE_URL = '/library/patristic/npnf211.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — Cassian's
// twelve Books of the Institutes, his twenty-four Conferences in their three
// Parts, and the seven Books on the Incarnation.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-11 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-11 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF211_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF211(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF211_TITLE, type: 'extra-biblical', language: 'en',
