...
export const ANF09_TITLE =
  'Ante-Nicene Fathers, Vol. 9: Recently Discovered Additions to Early Christian Literature';

const BUNDLE_URL = '/library/patristic/anf09.json';
...
async function loadBundle(): Promise<BundledANF09File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF09 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF09File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF09 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF09File): ParsedSource {
...
export async function installANF09(onProgress: (msg: string) => void): Promise<number> {
