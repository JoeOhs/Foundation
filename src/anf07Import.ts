...
export const ANF07_TITLE =
  'Ante-Nicene Fathers, Vol. 7: Fathers of the Third and Fourth Centuries';

const BUNDLE_URL = '/library/patristic/anf07.json';
...
async function loadBundle(): Promise<BundledANF07File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF07 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF07File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF07 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF07File): ParsedSource {
...
export async function installANF07(onProgress: (msg: string) => void): Promise<number> {
