...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF1-12 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF1-12 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF112_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF112(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF112_TITLE, type: 'extra-biblical', language: 'en',
