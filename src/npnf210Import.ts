...
export const NPNF210_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 10: Ambrose: Select Works and Letters';

const BUNDLE_URL = '/library/patristic/npnf210.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — each of
// Ambrose's treatises that divides into Books, so that De Officiis's three
// Books and De Fide's five sit under the treatise's name rather than beside
// it.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-10 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-10 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF210_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF210(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF210_TITLE, type: 'extra-biblical', language: 'en',
