...
export const NPNF205_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 5: Gregory of Nyssa: Dogmatic Treatises, Select Writings and Letters';

const BUNDLE_URL = '/library/patristic/npnf205.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — Against
// Eunomius and its twelve Books, inside Dogmatic Treatises. Series I never
// needed it: no volume there ran deeper than Work → Chapter.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-5 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-5 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF205_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF205(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF205_TITLE, type: 'extra-biblical', language: 'en',
