...
export const ANF05_TITLE =
  'Ante-Nicene Fathers, Vol. 5: Hippolytus, Cyprian, Caius, Novatian, Appendix';

const BUNDLE_URL = '/library/patristic/anf05.json';
...
async function loadBundle(): Promise<BundledANF05File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF05 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF05File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF05 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF05File): ParsedSource {
...
export async function installANF05(onProgress: (msg: string) => void): Promise<number> {
