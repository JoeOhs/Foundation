...
export const ANF03_TITLE =
  'Ante-Nicene Fathers, Vol. 3: Latin Christianity: Its Founder, Tertullian';

const BUNDLE_URL = '/library/patristic/anf03.json';
...
async function loadBundle(): Promise<BundledANF03File> {
  const res = await fetch(BUNDLE_URL);
  if (!res.ok) throw new Error(`Couldn't load the bundled ANF03 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledANF03File;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled ANF03 file is empty or malformed.');
  }
  return data;
}

function buildParsedSource(data: BundledANF03File): ParsedSource {
...
export async function installANF03(onProgress: (msg: string) => void): Promise<number> {
