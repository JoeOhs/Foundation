...
export const NPNF213_TITLE =
  'Nicene and Post-Nicene Fathers, Series II, Vol. 13: Gregory the Great (Part II), Ephraim Syrus, Aphrahat';

const BUNDLE_URL = '/library/patristic/npnf213.json';

interface BundledChapter { number: number; title: string; paragraphs: string[]; }
// `group` marks a work that sits inside a container of works — here only
// the three parts of Gwynn's Introductory Dissertation on Ephraim and
// Aphrahat.
...
async function loadBundle(): Promise<BundledNPNFFile> {
  ...
  if (!res.ok) throw new Error(`Couldn't load the bundled NPNF2-13 text (HTTP ${res.status})`);
  const data = (await res.json()) as BundledNPNFFile;
  if (!Array.isArray(data.authors) || data.authors.length === 0) {
    throw new Error('The bundled NPNF2-13 file is empty or malformed.');
  }
  return data;
}
...
function buildParsedSource(data: BundledNPNFFile): ParsedSource {
  ...
  return {
    suggestedTitle: NPNF213_TITLE, suggestedType: 'extra-biblical', structure: 'freeform',
...
export async function installNPNF213(onProgress: (msg: string) => void): Promise<number> {
  ...
  const sourceId = await insertParsedSource(parsed, {
    title: NPNF213_TITLE, type: 'extra-biblical', language: 'en',
