...
// Fathers, Series I, Volume 13 from CCEL, parses, strips footnotes, writes
// npnf113.json for Foundation's compound-work import.
//
// PROVENANCE — NPNF Series I, Vol. 13: Chrysostom: Homilies on Galatians,
...
// 1,576 are balanced, none nested and none self-closing, so the non-greedy
...
// across Series I — `id,n,place` (most volumes), `id,n`
// (Vol. 10) and `anchored,id,n,place` (Vol. 13) — so the strip is kept
// attribute-agnostic and each volume is checked rather than assumed.
//
// STRUCTURE — the deepest of the Chrysostom volumes, and the only one with
// div3. The eight epistles are NOT eight div1s: the source groups them in
// three, matching the original three-part publication — Galatians+Ephesians,
// Philippians+Colossians+Thessalonians, Timothy+Titus+Philemon. Within each,
// div2 is one epistle and div3 is one homily, so groupDiv2s() sees a Book
// container and gives every epistle its own work with its homilies as
// chapters. div3 titles are scripture ranges, all 114 already distinct, so
// divTitle() folding is cosmetic here rather than load-bearing — it adds the
// number each homily opens with ("Homily II. 1 Timothy 1:5-7") so this
// volume reads the same way as Vols 12 and 14.
