...
// Fathers, Series I, Volume 2 from CCEL, parses, strips footnotes, writes
// npnf102.json for Foundation's compound-work import.
//
// PROVENANCE — NPNF Series I, Vol. 2: Augustine: City of God, On Christian
// Doctrine. Edited by Philip Schaff. First published 1886–1889. Public domain.
//
...
function buildBundle(xml) {
  ...
  for (const div1 of div1s) {
    ...
    for (const div2 of div2s) {
      ...
      if (div3s.length > 0) {
        ...
        for (const div3 of div3s) {
          ...
          if (paragraphs.length > 0) {
            chapters.push({ number: chapters.length + 1, title: chTitle || `Section ${chapters.length + 1}`, paragraphs });
        ...
        if (paragraphs.length > 0) {
          works.push({ title: workTitle, chapters: [{ number: 1, title: workTitle, paragraphs }] });
          totalSections += paragraphs.length;
        }
      }
    }
    ...
    if (div2s.length === 0) {
      ...
      if (paragraphs.length > 0) {
        works.push({ title: sectionName, chapters: [{ number: 1, title: sectionName, paragraphs }] });
        totalSections += paragraphs.length;
      }
    }
  ...
  return {
    metadata: {
      title: 'Nicene and Post-Nicene Fathers, Series I, Vol. 2: Augustine: City of God, On Christian Doctrine',
      series: 'Nicene and Post-Nicene Fathers, Series I',
      volume: 2,
      editor: 'Philip Schaff',
      source_url: 'https://ccel.org/ccel/schaff/npnf102',
...
function validate(bundle) {
  ...
  if (metadata.total_paragraphs < 500) {
    throw new Error(`Expected at least 500 paragraphs; got ${metadata.total_paragraphs}.`);
  }
  let tagLeaks = 0;
  for (const a of authors) {
    for (const w of a.works) {
      for (const ch of w.chapters) {
        for (const p of ch.paragraphs) { if (/<[a-z]\w*[\s>]/i.test(p)) tagLeaks++; }
      }
    }
  }
