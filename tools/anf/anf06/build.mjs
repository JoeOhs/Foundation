// Standalone data-prep script for Ante-Nicene Fathers, Volume 6 — same
// pattern as anf01/build.mjs. Downloads ThML XML from CCEL, parses the
// div hierarchy, strips editorial footnotes, and writes anf06.json.
//
// PROVENANCE — Ante-Nicene Fathers, Vol. 6: Gregory Thaumaturgus, Dionysius the Great, Julius Africanus, Anatolius, and Minor Writers, Methodius, Arnobius:
//  Edited by Alexander Roberts and James Donaldson; revised and
...
const OUTPUT_PATH = path.join(__dirname, 'anf06.json');
const DEPLOY_PATH = path.join(__dirname, '..', '..', '..', 'public', 'library', 'patristic', 'anf06.json');

const THML_URL = 'https://ccel.org/ccel/s/schaff/anf06.xml';
const USER_AGENT =
...
async function loadRaw(refetch) {
  const cachePath = path.join(RAW_DIR, 'anf06.xml');
  if (!refetch) {
    try {
...
function buildBundle(xml) {
  ...
  return {
    metadata: {
      title: 'Ante-Nicene Fathers, Vol. 6: Gregory Thaumaturgus, Dionysius the Great, Julius Africanus, Anatolius, and Minor Writers, Methodius, Arnobius',
      series: 'Ante-Nicene Fathers',
      volume: 6,
      editors: 'Alexander Roberts, James Donaldson; rev. A. Cleveland Coxe',
      source_url: 'https://ccel.org/ccel/schaff/anf06',
...
function validate(bundle) {
  ...
  if (authors.length < 2) {
    throw new Error(
      `Expected at least 2 author group(s); got ${authors.length}.`,
    );
  }
