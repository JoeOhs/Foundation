// Standalone data-prep script for Ante-Nicene Fathers, Volume 9 — same
// pattern as anf01/build.mjs. Downloads ThML XML from CCEL, parses the
// div hierarchy, strips editorial footnotes, and writes anf09.json.
//
...
const OUTPUT_PATH = path.join(__dirname, 'anf09.json');
const DEPLOY_PATH = path.join(__dirname, '..', '..', '..', 'public', 'library', 'patristic', 'anf09.json');

const THML_URL = 'https://ccel.org/ccel/s/schaff/anf09.xml';
const USER_AGENT =
...
async function loadRaw(refetch) {
  const cachePath = path.join(RAW_DIR, 'anf09.xml');
  if (!refetch) {
    try {
...
function buildBundle(xml) {
  ...
  return {
    metadata: {
      title: 'Ante-Nicene Fathers, Vol. 9: Recently Discovered Additions to Early Christian Literature',
      series: 'Ante-Nicene Fathers',
      volume: 9,
      editors: 'Alexander Roberts, James Donaldson; rev. A. Cleveland Coxe',
      source_url: 'https://ccel.org/ccel/schaff/anf09',
...
function validate(bundle) {
  ...
  if (metadata.total_paragraphs < 200) {
    throw new Error(
      `Expected at least 200 paragraphs; got ${metadata.total_paragraphs}.`,
    );
  }
