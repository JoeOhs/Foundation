// Standalone data-prep script for Ante-Nicene Fathers, Volume 3 — same
// pattern as anf01/build.mjs. Downloads ThML XML from CCEL, parses the
// div hierarchy, strips editorial footnotes, and writes anf03.json.
//
// PROVENANCE — Ante-Nicene Fathers, Vol. 3: Latin Christianity: Its Founder, Tertullian:
...
const OUTPUT_PATH = path.join(__dirname, 'anf03.json');
const DEPLOY_PATH = path.join(__dirname, '..', '..', '..', 'public', 'library', 'patristic', 'anf03.json');

const THML_URL = 'https://ccel.org/ccel/s/schaff/anf03.xml';
const USER_AGENT =
...
async function loadRaw(refetch) {
  const cachePath = path.join(RAW_DIR, 'anf03.xml');
  if (!refetch) {
    try {
...
function buildBundle(xml) {
  ...
  return {
    metadata: {
      title: 'Ante-Nicene Fathers, Vol. 3: Latin Christianity: Its Founder, Tertullian',
      series: 'Ante-Nicene Fathers',
      volume: 3,
      editors: 'Alexander Roberts, James Donaldson; rev. A. Cleveland Coxe',
      source_url: 'https://ccel.org/ccel/schaff/anf03',
