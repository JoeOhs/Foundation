// Standalone data-prep script for Ante-Nicene Fathers, Volume 2 — same
// pattern as anf01/build.mjs. Downloads ThML XML from CCEL, parses the
// div hierarchy, strips editorial footnotes, and writes anf02.json.
//
...
const OUTPUT_PATH = path.join(__dirname, 'anf02.json');
const DEPLOY_PATH = path.join(__dirname, '..', '..', '..', 'public', 'library', 'patristic', 'anf02.json');

const THML_URL = 'https://ccel.org/ccel/s/schaff/anf02.xml';
const USER_AGENT =
...
async function loadRaw(refetch) {
  const cachePath = path.join(RAW_DIR, 'anf02.xml');
  if (!refetch) {
    try {
...
function buildBundle(xml) {
  ...
  return {
    metadata: {
      title: 'Ante-Nicene Fathers, Vol. 2: Fathers of the Second Century',
      series: 'Ante-Nicene Fathers',
      volume: 2,
      editors: 'Alexander Roberts, James Donaldson; rev. A. Cleveland Coxe',
      source_url: 'https://ccel.org/ccel/schaff/anf02',
...
function validate(bundle) {
  ...
  if (authors.length < 3) {
    throw new Error(
      `Expected at least 3 author groups (Hermas, Tatian, Athenagoras, Theophilus, ` +
      `Clement of Alexandria); got ${authors.length}.`,
    );
  }
  if (metadata.total_paragraphs < 1000) {
    throw new Error(
      `Expected at least 1,000 paragraphs; got ${metadata.total_paragraphs}.`,
    );
  }
  // Check that Hermas's Shepherd is present with its distinctive structure
  const hermas = authors.find((a) => /hermas|pastor|shepherd/i.test(a.name));
  if (!hermas) {
    console.log('⚠ Hermas/Shepherd author group not found by name — checking works…');
  }
