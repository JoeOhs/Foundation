// Standalone data-prep script for Ante-Nicene Fathers, Volume 8 — same
// pattern as anf01/build.mjs. Downloads ThML XML from CCEL, parses the
// div hierarchy, strips editorial footnotes, and writes anf08.json.
//
...
const OUTPUT_PATH = path.join(__dirname, 'anf08.json');
const DEPLOY_PATH = path.join(__dirname, '..', '..', '..', 'public', 'library', 'patristic', 'anf08.json');

const THML_URL = 'https://ccel.org/ccel/s/schaff/anf08.xml';
const USER_AGENT =
...
async function loadRaw(refetch) {
  const cachePath = path.join(RAW_DIR, 'anf08.xml');
  if (!refetch) {
    try {
...
function buildBundle(xml) {
  ...
  return {
    metadata: {
      title: 'Ante-Nicene Fathers, Vol. 8: Fathers of the Third and Fourth Centuries (Continued)',
      series: 'Ante-Nicene Fathers',
      volume: 8,
      editors: 'Alexander Roberts, James Donaldson; rev. A. Cleveland Coxe',
      source_url: 'https://ccel.org/ccel/schaff/anf08',
