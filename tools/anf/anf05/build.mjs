// Standalone data-prep script for Ante-Nicene Fathers, Volume 5 — same
// pattern as anf01/build.mjs. Downloads ThML XML from CCEL, parses the
// div hierarchy, strips editorial footnotes, and writes anf05.json.
//
// PROVENANCE — Ante-Nicene Fathers, Vol. 5: Hippolytus, Cyprian, Caius, Novatian, Appendix:
...
const OUTPUT_PATH = path.join(__dirname, 'anf05.json');
const DEPLOY_PATH = path.join(__dirname, '..', '..', '..', 'public', 'library', 'patristic', 'anf05.json');

const THML_URL = 'https://ccel.org/ccel/s/schaff/anf05.xml';
const USER_AGENT =
...
async function loadRaw(refetch) {
  const cachePath = path.join(RAW_DIR, 'anf05.xml');
  if (!refetch) {
    try {
...
function buildBundle(xml) {
  ...
  return {
    metadata: {
      title: 'Ante-Nicene Fathers, Vol. 5: Hippolytus, Cyprian, Caius, Novatian, Appendix',
      series: 'Ante-Nicene Fathers',
      volume: 5,
      editors: 'Alexander Roberts, James Donaldson; rev. A. Cleveland Coxe',
      source_url: 'https://ccel.org/ccel/schaff/anf05',
