// Standalone data-prep script for Ante-Nicene Fathers, Volume 4 — same
// pattern as anf01/build.mjs. Downloads ThML XML from CCEL, parses the
// div hierarchy, strips editorial footnotes, and writes anf04.json.
//
// PROVENANCE — Ante-Nicene Fathers, Vol. 4: Tertullian, Part Fourth; Minucius Felix; Commodian; Origen, Parts First and Second:
...
const OUTPUT_PATH = path.join(__dirname, 'anf04.json');
const DEPLOY_PATH = path.join(__dirname, '..', '..', '..', 'public', 'library', 'patristic', 'anf04.json');

const THML_URL = 'https://ccel.org/ccel/s/schaff/anf04.xml';
const USER_AGENT =
...
async function loadRaw(refetch) {
  const cachePath = path.join(RAW_DIR, 'anf04.xml');
  if (!refetch) {
    try {
...
function buildBundle(xml) {
  ...
  return {
    metadata: {
      title: 'Ante-Nicene Fathers, Vol. 4: Tertullian, Part Fourth; Minucius Felix; Commodian; Origen, Parts First and Second',
      series: 'Ante-Nicene Fathers',
      volume: 4,
      editors: 'Alexander Roberts, James Donaldson; rev. A. Cleveland Coxe',
      source_url: 'https://ccel.org/ccel/schaff/anf04',
