...
// Fathers, Series I, Volume 14 from CCEL, parses, strips footnotes, writes
// npnf114.json for Foundation's compound-work import.
//
// PROVENANCE — NPNF Series I, Vol. 14: Chrysostom: Homilies on the Gospel of
// St. John, Homilies on Hebrews.
...
// 3,526 are balanced, none nested and none self-closing, so the non-greedy
...
// across Series I — `id,n,place` (most volumes, including this one), `id,n`
...
//
// STRUCTURE — two div1 works, John (90 div2) and Hebrews (38 div2), both
// flat runs with no div3. This is the volume that forced divTitle()'s
// leading-paragraph fallback: its titles are scripture references that
// repeat ("John 1.1" three times running) and its shorttitle is empty, so
// neither Vol. 10's trick nor the raw title can tell those homilies apart.
// The number comes from each homily's own opening line ("Homily II.").
...
const VOLUME_TITLE =
  'Nicene and Post-Nicene Fathers, Series I, Vol. 14: Chrysostom: Homilies on the Gospel of St. John, Homilies on Hebrews';
